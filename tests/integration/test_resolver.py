...
def test_resolve_numpy_simple():
    ...
    opts = Options()
    result = asyncio.get_event_loop().run_until_complete(resolve_dependency_plan(reqs, cutoff, opts))
    assert any(name == "numpy" and "<=1.20.0" in str(spec) for name, spec in result.constraints.items())
